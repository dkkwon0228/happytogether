...
public class Canny {
    ...
    /*
    private static BufferedImage incluiMoldura(BufferedImage im, int largura) {
    ...
    public static void main(String[] args) {
        ...
        int largura = 10;
        detector.setGaussianKernelRadius(1.0f);
        detector.setLowThreshold(7.0f);
        detector.setHighThreshold(8.0f);
        String files = "../data/proc/obj%d__%d.png";
        for (int i = 1; i <= 20; i++)
        for (int j = 0; j <= 69; j++) {
            String fn = String.format(files,i,j);
            BufferedImage im = fe.loadImage(fn);
            im = incluiMoldura(im, largura);
            detector.setSourceImage(im);
            detector.process();
            BufferedImage edges = detector.getEdgesImage();
            String fout = String.format("zzz-%02d-%02d.png", i,j);
            //fe.saveImage(edges, fout);
            //thinning
            Thinning thin = new Thinning(edges);
            fe.saveImage(thin.getBufferedImage(), fout);
        }
    }
    */ 
}
