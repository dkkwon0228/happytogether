...
http://www.cfm.brown.edu/people/maxia/CS252/
...
public class TriangDelaunay implements DelaunayConstant {
  ...
  public TriangDelaunay(int siteNum)
  {
...
public void triangularizaG(Graph graph) {
	...
	int totv = graph.getNumVertices();
	numOfSites = totv;
    GeomSites[] s = new GeomSites[numOfSites];
	Vertex[] refv = graph.getVertices();
    ...
    while(quadEdgeNum > 0) {
        ...
        for(int k=0;k<2;k++) {
            if (q.rotSym().orig() == null && q.lnext().dest() == q.lprev().orig()) {    
                ...
                if (!adj[id1][id2]) graph.addEdge(v1, v2);
                if (!adj[id2][id1]) graph.addEdge(v2, v1);

                if (!adj[id2][id3]) graph.addEdge(v2, v3);
                if (!adj[id3][id2]) graph.addEdge(v3, v2);

                if (!adj[id1][id3]) graph.addEdge(v1, v3);
                if (!adj[id3][id1]) graph.addEdge(v3, v1);
  ...
  public static void main(String[] args) 
  {
    ...
    String sitesFileName=parser(args);  
    // if sitesFileName is  null generate sites randomly.
    // parser sets runs and numOfSites;
    if(sitesFileName=="wrong") return; // something goes run.

    // generate the sites (randomly or from the specified file)
    GeomSites[] s = generateSites(sitesFileName);
    if(s==null) return; //something goes wrong, sites did not generated.
  
  
    TriangDelaunay del = new TriangDelaunay(s.length);

    // Delaunay triangulation
    if (runs==DIVIDE_AND_CONQUER) 
    {
      ...
      del.DivideAndConquer(s,0,s.length);
      ...
      if (args[i].startsWith("-")) 
      {
        switch(args[i].charAt(1))
        {
          ...
          case 'r':
            ...
            break;
          default:
            System.out.println("usage: java TriangDelaunay -[db] [-r#] [inpu_file]");
