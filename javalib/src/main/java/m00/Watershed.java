...
-> evita revisitar os elementos jah examinados
...
public class Watershed {
    ...
    private BufferedImage imRGB = null;
...
Conversion opConv2 = new Conversion(im, true);
this.imRGB = opConv2.imgConverted(); // 2009jul23: salva imagem RGB para sumir com linhas de watershed
...
*/
    ...
    public Watershed(double[][] mtInp, BufferedImage im) { //sem blur: para reducao de regioes do wshed (ReducedWshed)
//se imagem grayscale, replica para tres canais RGB
Conversion opConv2 = new Conversion(im, true);
this.imRGB = opConv2.imgConverted(); // 2009jul23: salva imagem RGB para sumir com linhas de watershed
        this.w = mtInp.length;
...
System.out.println("Wshed..............");
*/
//2009jul23: ordena crescente por nivel de cinza
LinkedList[] laux = new LinkedList[256];
for (int ii = 0; ii < 256; ii++)
laux[ii] = new LinkedList();
...
int graylevel = (int)Math.floor(mtInp[i][j]);
laux[graylevel].add(new Pixel(i, j, graylevel));
...
*/
        ...
        this.wshedline2 = calcContorno2(); //pixels cujo rotulo eh WSHED
        /*
Elapsed elap = new Elapsed();
elap.setFirstEvent();
System.out.println("Wshed...........");
...
*/
    ...
    public void vanishWshed() {
        ...
        */
        //2009jul23: demora muito e nao entendi o que faz...
        /*
        ...
        for (int i=0; i<w; i++) 
            for (int j=0; j<h; j++)
                ...
                */
        //2009jul23: some com linhas do watershed, indo para o vizinho mais parecido (cor)
            ...
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++) 
                    if (mtLab[x][y] == WSHED) {
                        ...
                        float[] lab = fe.rgb2cielab(r, g, b);
                        Pixel min = null; //viz mais parecido
                        float mindist = 9999.9f;
                        for (int v=0; v<4; v++) {
                            Pixel q = new Pixel(x+cts.viz4x[v], y+cts.viz4y[v], 0);
                            if (insideImg(q)) {
                                int r2 = inp.getSample(q.getx(),q.gety(),0);
                                int g2 = inp.getSample(q.getx(),q.gety(),1);
                                int b2 = inp.getSample(q.getx(),q.gety(),2);
                                float[] lab2 = fe.rgb2cielab(r2, g2, b2);
                                float dist = fe.euclideanDistance(lab,lab2,3)/100.0f;
                                if (dist < mindist) {
                                    min = q;
                                    mindist = dist;
                                }
                        ...
                        if (min != null)
                            mtLab[x][y] = mtLab[min.getx()][min.gety()];
                        else {
                            System.out.println("Erro1: vanishWshed()");
                            System.exit(1);
                        }
                    }
            for (int y = 0; y < h; y++)
                for (int x = 0; x < w; x++) 
                    if (mtLab[x][y] == WSHED) {
                    /*
                        System.out.println("Erro2: vanishWshed()");
                        System.exit(1);
                        */
                    }
                    else
